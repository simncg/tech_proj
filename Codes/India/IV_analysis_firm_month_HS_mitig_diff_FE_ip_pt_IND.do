...
*              regressions we include a different set of fixed effects (ip + pt)
...
foreach data of local imp_exp {
	...
	* Run regressions for model that measures if tech adoption affects trade outcomes 
	foreach iv of varlist  ltg dspg uspg ltb dspb uspb  {	
		gen `iv'covid = `iv'*sti  
		eststo `y'`iv'covid: ivreghdfe `y' (techcovid = `iv'covid), absorb(ip pt) cluster(company_id hs6) savefirst savefprefix(f`y'`iv') 	
...
foreach y of local dep_vars {
	...
	* Creating tex file with the table for first stage
	cap file close fh 
	file open fh using "$path_outputs/India/regressions_results/tech_mitigation_covid_model/first_stage_log_iv_`y'_mitig_FE_ip_pt.tex", write replace  // Opening latex file for first stage
...
foreach y of local dep_vars {
	...
	* Creating tex file with the table for first stage
	cap file close fh 
	file open fh using "$path_outputs/India/regressions_results/tech_mitigation_covid_model/second_stage_log_iv_`y'_mitig_FE_ip_pt.tex", write replace  // Opening latex file for first stage
