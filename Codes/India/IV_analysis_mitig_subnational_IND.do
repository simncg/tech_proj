...
foreach data of local imp_exp {
	...
	// Take logs in the COVID variables
...
foreach y of local dep_vars {
	...
	* Adding coefficients and significance level stars 
	file write fh " $ covid_{it} \cdot Z_i $ " "&" %7.6fc (`b_ltg') "`star_ltg'" "&"  %7.6fc (`b_dspg') "`star_dspg'" "&"  %7.6fc (`b_uspg') "`star_uspg'" "&"  %7.6fc (`b_ltb') "`star_ltb'" "&"  %7.6fc (`b_dspb') "`star_dspb'" "&"  %7.6fc (`b_uspb') "`star_uspb'" "\\" _n		
...
foreach y of local dep_vars {
	...
	* Creating tex file with the table for first stage
	cap file close fh 
	file open fh using "$path_outputs/India/regressions_results/tech_mitigation_covid_model/second_stage_log_iv_`y'_subnational_mitig.tex", write replace  // Opening latex file for first stage
