...
#                                                                           #
# This program formats the Indian technology data (BuiltWith) to merge it   #
# with Panjiva and Aberdeen datasets. Also, it creates lags of the          #
...
tech_data <- 
  tech_data %>% 
  # Identify technology adoption date (first_adopted variable) as a character variable
  left_join(
    tech_data %>% 
      select(-LI, -FI) %>% 
  ...
  group_by(company_id) %>% 
  mutate(across(c(ecom, payrobust), 
                list(t_1 = function(x){lag(x, 1)}, 
