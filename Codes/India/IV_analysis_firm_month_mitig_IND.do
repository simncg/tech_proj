...
foreach data of local imp_exp {
	...
	* Rename to avoid long names in estimates store
	...
	rename adopted_pay_or_ecom_before_2019 tech   // technology variable (pre-2019 adoption of technolgy)
...
foreach y of local dep_vars {
	...
	* Creating tex file with the table for first stage
	cap file close fh 
	file open fh using "$path_outputs/India/regressions_results/firm_month_covid_mitigation_model/first_stage_log_iv_`y'_mitig_firm_month.tex", write replace  // Opening latex file for first stage
...
foreach y of local dep_vars {
	...
	* Creating tex file with the table for first stage
	cap file close fh 
	file open fh using "$path_outputs/India/regressions_results/firm_month_covid_mitigation_model/second_stage_log_iv_`y'_mitig_firm_month.tex", write replace  // Opening latex file for first stage
