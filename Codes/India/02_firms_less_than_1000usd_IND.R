...
export_data_firm_month <-
  fread("../../Data/India/processed_data/export_summaries_by_firm_month_HS_code_complete.csv") %>% 
  filter(!str_starts(hs6, "27"), 
         date >= ymd("2018-01-01"), # We take advantage that we have 2018, 2019, 2020, 2021  complete for India to do this filter
         date < ymd("2022-01-01")
         ) %>% 
...
import_data_firm_month <-
  fread("../../Data/India/processed_data/import_summaries_by_firm_month_HS_code_complete.csv")%>% 
  filter(!str_starts(hs6, "27"), 
         date >= ymd("2018-01-01"), # We take advantage that we have 2018, 2019, 2020 and 2021 complete for India
         date < ymd("2022-01-01")
         ) %>% 
  rename(company_id = domestic_company_id)
...
# Save results 
write_csv(less_than_1000USD_export, "../../Data/India/processed_data/export_firms_trade_less_than_1000usd.csv")
write_csv(less_than_1000USD_import, "../../Data/India/processed_data/import_firms_trade_less_than_1000usd.csv")
