...
*              set of FE (we use i + pt FE) compared to the initial set of 
*              FE (FE_i, FE_p, FE_t). . 
*                 
* Inputs: 
* 
	...
	* Run regressions for model that measures if tech adoption affects trade outcomes 
	foreach iv of varlist  ltg dspg uspg ltb dspb uspb  {
		foreach covid_var of varlist cas sti dwc {
			...
			eststo `y'`iv'`covid_var': ivreghdfe `y' (tech = `iv'_`covid_var'), absorb(company_id pt) cluster(company_id hs6) savefirst savefprefix(f`y'`iv'`covid_var') 	
...
foreach y of local dep_vars {
	...
	* Creating tex file with the table for first stage
	cap file close fh 
	file open fh using "$path_outputs/India/regressions_results/products_model/first_stage_log_iv_`y'_FE_i_pt.tex", write replace  // Opening latex file for first stage
...
foreach y of local dep_vars {
	...
	* Creating tex file with the table for first stage
	cap file close fh 
	file open fh using "$path_outputs/India/regressions_results/products_model/second_stage_log_iv_`y'_FE_i_pt.tex", write replace  // Opening latex file for first stage
