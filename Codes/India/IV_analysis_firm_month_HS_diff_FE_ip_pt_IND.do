...
*
* Description: This do-file runs the IV analysis for India using a different 
*              set of FE (we use ip + pt FE) compared to the initial set of 
*              FE (FE_i, FE_p, FE_t). 
*                 
* Inputs: 
* 
...
foreach data of local imp_exp {
	...
	* Generate variable with combination between firm and product 
	gen ip = company_id + "_" + string(hs6) 
	label var ip "Firm-product combination"
	...
	* Run regressions for model that measures if tech adoption affects trade outcomes 
	foreach iv of varlist  ltg dspg uspg ltb dspb uspb  {
		foreach covid_var of varlist cas sti dwc {
			...
			eststo `y'`iv'`covid_var': ivreghdfe `y' (tech = `iv'_`covid_var'), absorb(ip pt) cluster(company_id hs6) savefirst savefprefix(f`y'`iv'`covid_var') 	
...
foreach y of local dep_vars {
	...
	* Creating tex file with the table for first stage
	cap file close fh 
	file open fh using "$path_outputs/India/regressions_results/products_model/first_stage_log_iv_`y'_FE_ip_pt.tex", write replace  // Opening latex file for first stage
...
foreach y of local dep_vars {
	...
	* Creating tex file with the table for first stage
	cap file close fh 
	file open fh using "$path_outputs/India/regressions_results/products_model/second_stage_log_iv_`y'_FE_ip_pt.tex", write replace  // Opening latex file for first stage
