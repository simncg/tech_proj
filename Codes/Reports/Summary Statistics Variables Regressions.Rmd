...
# Builtwith India 
tech_MEX   <- fread("../../Data/Mexico/raw_data/Builtwith_Mexico_no_drop_long.csv") %>% 
  select(our_ID, tech) %>% 
  # For the moment, we are only going to analyze e-payment/e-commerce technologies
  filter(tech %in% c("payrobust_nod", "ecom_nod")) %>% 
  rename(company_id = our_ID) %>% 
...
# Firms matched to Aberdeen 
aberdeen_MEX <- read_csv("../../Data/Mexico/raw_data/matched_Aberdeen_to_Panjiva_data_Mexico.csv") %>%
  rename(company_id = our_ID) %>% 
  distinct(company_id)


# Panjiva Imports Data 
imports_MEX <-read_parquet("../../Data/Mexico/processed_data/import_summaries_by_firm_month_HS_code_complete.parquet", 
...
# Panjiva Exports Data 
exports_MEX <-read_parquet("../../Data/Mexico/processed_data/export_summaries_by_firm_month_HS_code_complete.parquet", 
                           col_select = c("domestic_company_id", "year", 'month', "date", 
...
kbl(table_IND %>% select(-Data), 
  ...
  add_footnote(c("Only data from July 2018 onwards is included."))
...
kbl(table_MEX %>% select(-Data), 
    caption = "Mexico - Panjiva total imports and total exports matching rates to BuiltWith and Aberdeen datasets by year", centering = T, booktabs = T, align = "c") %>%
  kable_classic() %>%
  kable_styling(latex_options = "HOLD_position") %>% 
  pack_rows("Panjiva Imports", 1, 4) %>%
  pack_rows("Panjiva Exports", 5, 8)%>% 
...
kbl(table_MEX %>% select(-Data), 
    caption = "Mexico - Panjiva firms matching rates to BuiltWith and Aberdeen datasets by year", centering = T, booktabs = T, align = "c") %>%
  kable_classic() %>%
  kable_styling(latex_options = "HOLD_position") %>% 
  pack_rows("Panjiva Imports", 1, 4) %>%
  pack_rows("Panjiva Exports", 5, 8)%>% 
...
kbl(table_MEX %>% select(-Data), 
    caption = "Mexico - Panjiva transactions at the Firm-Month-HS6 level matching rates to BuiltWith and Aberdeen datasets by year", centering = T, booktabs = T, align = "c") %>%
  kable_classic() %>%
  kable_styling(latex_options = "HOLD_position") %>% 
  pack_rows("Panjiva Imports", 1, 4) %>%
  pack_rows("Panjiva Exports", 5, 8)%>% 
...
dep_vars<-
  ...
  # Mexico
    bind_rows(pay_ecom_export_data_MEX %>% 
      as.data.frame() %>% 
      mutate(export = exp(log_export)) %>% 
      select(export) %>% 
      summarize_all(list(Min=min, p25 = ~quantile(., 0.25), Median = median, Mean = mean, p75 = ~quantile(., 0.75), Max = max, SD = sd)) %>% 
      bind_rows(pay_ecom_import_data_MEX %>% 
                  as.data.frame() %>% 
                  mutate(import = exp(log_import)) %>% 
                  select(import) %>% 
                  summarize_all(list(Min=min, p25 = ~quantile(., 0.25), Median = median, 
...
rownames(dep_vars)<-c("Exports", "Imports", "Exports ", "Imports ", "Exports  ", "Imports  ")
...
rownames(prods_tech)<-c("Exports", "Imports", "Exports ", "Imports ", "Exports  ", "Imports  ")
...
n_firms<-
  ...
  as.data.frame() %>% 
  distinct(company_id) %>% 
  summarize(`Exp. Firms` = n()) %>% 
  # Indonesia Imports 
  bind_cols(
   pay_ecom_import_data_IDN %>% 
    as.data.frame() %>% 
    distinct(company_id) %>% 
    summarize(`Imp. Firms` = n()) 
  ) %>% 
  # India Exports
  ...
  # Mexico Exports
  bind_cols(pay_ecom_export_data_MEX %>% 
    as.data.frame() %>% 
    distinct(company_id) %>% 
    summarize(`Exp. Firms  ` = n())
    ) %>% 
  # Mexico Imports
   bind_cols(pay_ecom_import_data_MEX %>% 
    as.data.frame() %>% 
    distinct(company_id) %>% 
    summarize(`Imp. Firms  ` = n())
  ) %>% 
...
kbl(n_firms, 
  ...
  kable_classic() %>%
  kable_styling(latex_options = "HOLD_position") %>% 
  add_header_above(c("Indonesia" = 2, "India" = 2, "Mexico" = 2))
...
# Indonesia exports and imports datasets used in the model that measures if tech adoption mitigates COVID impacts
...
# India exports and imports datasets used in the model that measures if tech adoption mitigates COVID impacts
...
# Mexico exports and imports datasets used in the model that measures if tech adoption mitigates COVID impacts
import_mitig_data_MEX<-fread("../../Data/Mexico/processed_data/imports_tech_mitigation_model_MEX.csv") %>% 
  mutate(month_mean_stringency_index = replace_na(month_mean_stringency_index, 0))%>% 
  as.data.frame()

export_mitig_data_MEX<-fread("../../Data/Mexico/processed_data/exports_tech_mitigation_model_MEX.csv") %>% 
  mutate(month_mean_stringency_index = replace_na(month_mean_stringency_index, 0))%>% 
  as.data.frame()
...
dep_vars<-
  ...
  # India
    ...
    # Mexico
    bind_rows(export_mitig_data_MEX %>% 
      as.data.frame() %>% 
      mutate(export = exp(log_export)) %>% 
      select(export) %>% 
      summarize_all(list(Min=min, p25 = ~quantile(., 0.25), Median = median, Mean = mean, p75 = ~quantile(., 0.75), Max = max, SD = sd)) %>% 
      bind_rows(import_mitig_data_MEX %>% 
                  as.data.frame() %>% 
                  mutate(import = exp(log_import)) %>% 
                  select(import) %>% 
                  summarize_all(list(Min=min, p25 = ~quantile(., 0.25), Median = median, 
...
prods_tech<-
  ...
  export_mitig_data_IDN %>% 
  as.data.frame() %>% 
  select(tech = adopted_pay_or_ecom_before_2020, Ebay_tradable, China_E_commerce, cons_BEC, durable_BEC) %>% 
  na.omit() %>% 
  summarize_all(list(Mean = mean)) %>% 
  pivot_longer(cols = everything(), 
               names_to = c("variable", "statistic"),
...
prods_tech<-as.data.frame(prods_tech)
rownames(prods_tech)<-c("Exports", "Imports", "Exports ", "Imports ", "Exports  ", "Imports  ")
colnames(prods_tech)<-c("Firm technology\nadoption pre-2020", "e-Bay tradable", "China E-commerce", "Consumable", "Durable")

kbl(prods_tech, 
    caption = "Proportion of Observations with Dummy Variables Equal to 1 for technology adopted before 2020, e-Bay Tradable, China E-commerce, Consumable, and Durable Product Categories - Regression Sample", centering = T, booktabs = T, align = "c") %>%
  kable_classic() %>%
  kable_styling(latex_options = "HOLD_position") %>% 
  pack_rows("Indonesia", 1, 2) %>%
  pack_rows("India", 3, 4) %>% 
...
rownames(stringency_index)<-c("Exports", "Imports", "Exports ", "Imports ", "Exports  ", "Imports  ")
...
n_firms<-
  ...
  # Mexico Exports
  bind_cols(export_mitig_data_MEX %>% 
    as.data.frame() %>% 
    distinct(company_id) %>% 
    summarize(`Exp. Firms  ` = n())
    ) %>% 
  # Mexico Imports
   bind_cols(import_mitig_data_MEX %>% 
    as.data.frame() %>% 
    distinct(company_id) %>% 
    summarize(`Imp. Firms  ` = n())
  ) %>% 
...
# Extensive margin imports and exports dataset for Indonesia
import_data_IDN<-read_parquet("../../Data/Indonesia/processed_data/imports_grid_product_model_IDN.parquet",
                            col_select = c("Ebay_tradable", "China_E_commerce", "cons_BEC", "durable_BEC",
                                           "company_id", "hs6", "date_character", "import_dummy",
                                           "pay_or_ecomnod_t_2"))


export_data_IDN<-read_parquet("../../Data/Indonesia/processed_data/exports_grid_product_model_IDN.parquet",
                            col_select = c("Ebay_tradable", "China_E_commerce", "cons_BEC", "durable_BEC",
                                           "company_id", "hs6", "date_character", "export_dummy",
                                           "pay_or_ecomnod_t_2"))

# Extensive margin imports and exports dataset for India
...
# Extensive margin imports and exports dataset for Mexico 
import_data_MEX<-read_parquet("../../Data/Mexico/processed_data/imports_grid_product_model_MEX.parquet",
                            col_select = c("Ebay_tradable", "China_E_commerce", "cons_BEC", "durable_BEC",
                                           "company_id", "hs6", "date_character", "import_dummy",
                                           "pay_or_ecomnod_t_2"))


export_data_MEX<-read_parquet("../../Data/Mexico/processed_data/exports_grid_product_model_MEX.parquet",
                            col_select = c("Ebay_tradable", "China_E_commerce", "cons_BEC", "durable_BEC",
                                           "company_id", "hs6", "date_character", "export_dummy",
                                           "pay_or_ecomnod_t_2"))
...
colnames(prop_zeros)<-c("Exports", "Imports", "Exports", "Imports", "Exports", "Imports")
...
kbl(prop_zeros, 
  ...
  kable_classic() %>%
  kable_styling(latex_options = "HOLD_position") %>% 
  add_header_above(c("Indonesia Expanded Datasets" = 2, "India Expanded Datasets" = 2, "Mexico Expanded Datasets" = 2))
...
# Extensive margin imports and exports datasets for India for the model that measures if tech adoption affects trade outcomes
...
# Extensive margin imports and exports datasets for Mexico for the model that measures if tech adoption affects trade outcomes
import_data_MEX<-read_parquet("../../Data/Mexico/processed_data/imports_grid_mitig_model_MEX.parquet",
                            col_select = c("Ebay_tradable", "China_E_commerce", "cons_BEC", "durable_BEC",
                                           "company_id", "hs6", "date_character", "import_dummy",
                                           "pay_or_ecomnod_t_2"))


export_data_MEX<-read_parquet("../../Data/Mexico/processed_data/exports_grid_mitig_model_MEX.parquet",
                            col_select = c("Ebay_tradable", "China_E_commerce", "cons_BEC", "durable_BEC",
                                           "company_id", "hs6", "date_character", "export_dummy",
                                           "pay_or_ecomnod_t_2"))
