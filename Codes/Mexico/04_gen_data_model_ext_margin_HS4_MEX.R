...
hs_data <- read_csv("../../Data/Extra Data/HS_code_classifications.csv")%>% 
  ...
  rename(hs6 = hs_2017, letter_credit_use = LCInt)
...
## Exports data for model that measures if tech adoption affects trade outcomes (after processed in gen_data_model_products_MEX.R) ----
exports_data_MEX<- fread("../../Data/Mexico/processed_data/exports_product_model_MEX.csv") %>% 
...
grid_firm_hs6_exp_MEX<- exports_data_MEX %>% 
...
exports_grid_MEX<- 
  CJ(firm_feas_hs6 = grid_firm_hs6_exp_MEX$firm_hs6, 
     date = unique(exports_data_MEX$date)) %>% 
  separate(col = firm_feas_hs6, into = c("company_id", "hs6"), sep = "-") %>% 
  # Join export dummy variable by company, month, hs6
  left_join(exports_data_MEX %>% 
...
write_parquet(exports_grid_MEX, "../../Data/Mexico/processed_data/exports_grid_product_model_MEX.parquet")
rm(exports_grid_MEX, grid_firm_hs6_exp_MEX, exports_data_MEX) # Remove objects to free space 
gc()

## Imports data for model that measures if tech adoption affects trade outcomes (after processed in gen_data_model_products_MEX.R) ----
imports_data_MEX<- fread("../../Data/Mexico/processed_data/imports_product_model_MEX.csv") %>% 
  arrange(company_id, date, hs6) %>% 
  mutate(hs6  = str_pad(hs6, 6, "left", "0"), 
         hs4 = substr(hs6, 1, 4)) %>% 
  relocate(date, hs6, hs4, company_id) %>% 
...
## Create all possible combinations between firms and FEASIBLE HS6 based on the HS4 code -----
grid_firm_hs6_imp_MEX<- imports_data_MEX %>% 
  select(company_id, hs4) %>% 
  # When a firm exports an HS6 product that belongs to a specific HS4 code, an observation is created for the corresponding firm-HS4 pair.
  distinct() %>% 
  # Expand the dataset to include all possible combinations between the firm and every HS6 code within the HS4 code that the firm exported.
...
imports_grid_MEX<- 
  CJ(firm_feas_hs6 = grid_firm_hs6_imp_MEX$firm_hs6, 
     date = unique(imports_data_MEX$date)) %>% 
  separate(col = firm_feas_hs6, into = c("company_id", "hs6"), sep = "-") %>% 
  # Join import dummy variable by company, month, hs6
  left_join(imports_data_MEX %>% 
...
write_parquet(imports_grid_MEX, "../../Data/Mexico/processed_data/imports_grid_product_model_MEX.parquet")
rm(imports_grid_MEX, grid_firm_hs6_imp_MEX, imports_data_MEX) # Remove objects to free space 
gc()
...
exports_data_mitig_MEX <-fread("../../Data/Mexico/processed_data/exports_tech_mitigation_model_MEX.csv") %>%
  arrange(company_id, date, hs6) %>% 
  mutate(hs6  = str_pad(hs6, 6, "left", "0"), 
         hs4 = substr(hs6, 1, 4)) %>% 
  relocate(date, hs6, hs4, company_id) %>% 
...
## Create all possible combinations between firms and FEASIBLE HS6 based on the HS4 code ----
grid_firm_hs6_exp_mitig_MEX<- exports_data_mitig_MEX %>% 
  select(company_id, hs4) %>% 
  # When a firm exports an HS6 product that belongs to a specific HS4 code, an observation is created for the corresponding firm-HS4 pair.
  distinct() %>% 
  # Expand the dataset to include all possible combinations between the firm and every HS6 code within the HS4 code that the firm exported.
...
exports_grid_mitig_MEX<- 
  CJ(firm_feas_hs6 = grid_firm_hs6_exp_mitig_MEX$firm_hs6, 
     date = unique(exports_data_mitig_MEX$date)) %>% 
  separate(col = firm_feas_hs6, into = c("company_id", "hs6"), sep = "-") %>% 
  # Join export dummy variable by company, month, hs6
  left_join(exports_data_mitig_MEX %>% 
              select(company_id, date, hs6, export_dummy),
            by = c('company_id', "date", "hs6")) %>% 
  # If missing, then product not imported by the company in that month
  mutate(export_dummy = ifelse(is.na(export_dummy), FALSE, export_dummy)) %>%
...
rm(exports_grid_mitig_MEX, grid_firm_hs6_exp_mitig_MEX, exports_data_mitig_MEX) # Remove objects to free space 
gc()



## Read imports data for regressions of the model that measures if tech adoption mitigated COVID impacts ----
imports_data_mitig_MEX <-fread("../../Data/Mexico/processed_data/imports_tech_mitigation_model_MEX.csv") %>%
  arrange(company_id, date, hs6) %>% 
  mutate(hs6  = str_pad(hs6, 6, "left", "0"), 
         hs4 = substr(hs6, 1, 4)) %>% 
  relocate(date, hs6, hs4, company_id) %>% 
...
## Create all possible combinations between firms and FEASIBLE HS6 based on the HS4 code ----
grid_firm_hs6_imp_mitig_MEX<- imports_data_mitig_MEX %>% 
  select(company_id, hs4) %>% 
  # When a firm exports an HS6 product that belongs to a specific HS4 code, an observation is created for the corresponding firm-HS4 pair.
  distinct() %>% 
  # Expand the dataset to include all possible combinations between the firm and every HS6 code within the HS4 code that the firm exported.
...
imports_grid_mitig_MEX<- 
  CJ(firm_feas_hs6 = grid_firm_hs6_imp_mitig_MEX$firm_hs6, 
     date = unique(imports_data_mitig_MEX$date)) %>% 
  separate(col = firm_feas_hs6, into = c("company_id", "hs6"), sep = "-") %>% 
  # Join import dummy variable by company, month, hs6
  left_join(imports_data_mitig_MEX %>% 
              select(company_id, date, hs6, import_dummy),
            by = c('company_id', "date", "hs6")) %>% 
  # If missing, then product not imported by the company in that month
  mutate(import_dummy = ifelse(is.na(import_dummy), FALSE, import_dummy)) %>%
...
rm(imports_grid_mitig_MEX, grid_firm_hs6_imp_mitig_MEX, imports_data_mitig_MEX) # Remove objects to free space 
gc()