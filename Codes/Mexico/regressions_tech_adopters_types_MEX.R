...
#                                                                           #
#                                                                           #
# Analysis of Mexican technology adopters: Regressions for the pre-Covid 
# period  
#                                                                           #
#                                                                           #                                                                                                                                                             #
#===========================================================================#
...
import_data<- fread("../../Data/Mexico/processed_data/imports_tech_mitigation_model_MEX.csv")
...
export_data<- fread("../../Data/Mexico/processed_data/exports_tech_mitigation_model_MEX.csv")
...
table_reg_all_adopters_MEX<-
  modelsummary(all_adopters_types_models,
               ...
               title = "Mexico - Regression Results for COVID, Old and Never Adopters - Pre-Covid Period.") %>% 
...
table_reg_old_covid_adopters_MEX<-
  modelsummary(old_covid_adopters_types_models,
               ...
               add_rows = FE, 
               align = paste(c("l", rep("c", 2)), sep="", collapse=""), 
               title = "Mexico - Regression Results for COVID and Old Adopters - Pre-Covid Period.") %>% 
  add_header_above(c(" " = 1, "Dependent Variables" = 2)) %>% 
  kable_styling(latex_options = c("HOLD_position")) %>% 
...
table_reg_covid_adopters_MEX<-
  modelsummary(covid_adopters_types_models,
               ...
               add_rows = FE, 
               align = paste(c("l", rep("c", 2)), sep="", collapse=""), 
               title = "Mexico - Regression Results for COVID Adopters - Pre-Covid Period.") %>% 
  add_header_above(c(" " = 1, "Dependent Variables" = 2)) %>% 
  kable_styling(latex_options = c("HOLD_position")) %>% 
...
table_reg_old_adopters_MEX<-
  modelsummary(old_adopters_types_models,
               ...
               add_rows = FE, 
               align = paste(c("l", rep("c", 2)), sep="", collapse=""), 
               title = "Mexico - Regression Results for Old Adopters - Pre-Covid Period.") %>% 
  add_header_above(c(" " = 1, "Dependent Variables" = 2)) %>% 
  kable_styling(latex_options = c("HOLD_position")) %>% 
