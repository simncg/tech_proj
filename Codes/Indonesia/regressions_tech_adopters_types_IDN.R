...
#                                                                           #
#                                                                           #
# Analysis of Indonesian technology adopters: Regressions for the pre-Covid # 
# period                                                                    #
#                                                                           #
#                                                                           #                                                                                                                                                             #
#===========================================================================#
...
import_data<- fread("../../Data/Indonesia/processed_data/imports_tech_mitigation_model_IDN.csv")
...
export_data<- fread("../../Data/Indonesia/processed_data/exports_tech_mitigation_model_IDN.csv")
...
table_reg_all_adopters_IDN<-
  modelsummary(all_adopters_types_models,
               ...
               title = "Indonesia - Regression Results for COVID, Old and Never Adopters - Pre-Covid Period.") %>% 
...
table_reg_old_covid_adopters_IDN<-
  modelsummary(old_covid_adopters_types_models,
               ...
               add_rows = FE, 
               align = paste(c("l", rep("c", 2)), sep="", collapse=""), 
               title = "Indonesia - Regression Results for COVID and Old Adopters - Pre-Covid Period.") %>% 
  add_header_above(c(" " = 1, "Dependent Variables" = 2)) %>% 
  kable_styling(latex_options = c("HOLD_position")) %>% 
...
table_reg_covid_adopters_IDN<-
  modelsummary(covid_adopters_types_models,
               ...
               add_rows = FE, 
               align = paste(c("l", rep("c", 2)), sep="", collapse=""), 
               title = "Indonesia - Regression Results for COVID Adopters - Pre-Covid Period.") %>% 
  add_header_above(c(" " = 1, "Dependent Variables" = 2)) %>% 
  kable_styling(latex_options = c("HOLD_position")) %>% 
...
table_reg_old_adopters_IDN<-
  modelsummary(old_adopters_types_models,
               ...
               add_rows = FE, 
               align = paste(c("l", rep("c", 2)), sep="", collapse=""), 
               title = "Indonesia - Regression Results for Old Adopters - Pre-Covid Period.") %>% 
  add_header_above(c(" " = 1, "Dependent Variables" = 2)) %>% 
  kable_styling(latex_options = c("HOLD_position")) %>% 
  footnote("Baseline category is pre-2018 adopter. The regression sample only includes the pre-Covid period. Clustered standard errors at the firm level", 
           threeparttable = TRUE) 
