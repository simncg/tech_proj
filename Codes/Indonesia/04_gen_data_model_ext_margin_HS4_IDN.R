...
tech_data_IDN<-read_parquet("../../Data/Indonesia/processed_data/tech_data_IDN.parquet", 
                            col_select = c("company_id", "date", "date_character", "pay_or_ecomnod_t_1",
                                           ...
                                           "adopted_pay_or_ecom_before_2020", "months_since_adoption", 
...
grid_firm_hs6_exp_IDN<- exports_data_IDN %>% 
...
exports_grid_IDN<- 
  CJ(firm_feas_hs6 = grid_firm_hs6_exp_IDN$firm_hs6, 
     date = unique(exports_data_IDN$date)) %>% 
  separate(col = firm_feas_hs6, into = c("company_id", "hs6"), sep = "-") %>% 
  # Join export dummy variable by company, month, hs6
  left_join(exports_data_IDN %>% 
...
write_parquet(exports_grid_IDN, "../../Data/Indonesia/processed_data/exports_grid_product_model_IDN.parquet")
rm(exports_grid_IDN, grid_firm_hs6_exp_IDN, exports_data_IDN) # Remove objects to free space 
gc()
...
## Create all possible combinations between firms and FEASIBLE HS6 based on the HS4 code -----
grid_firm_hs6_imp_IDN<- imports_data_IDN %>% 
  select(company_id, hs4) %>% 
  # When a firm exports an HS6 product that belongs to a specific HS4 code, an observation is created for the corresponding firm-HS4 pair.
  distinct() %>% 
  # Expand the dataset to include all possible combinations between the firm and every HS6 code within the HS4 code that the firm exported.
...
imports_grid_IDN<- 
  CJ(firm_feas_hs6 = grid_firm_hs6_imp_IDN$firm_hs6, 
     date = unique(imports_data_IDN$date)) %>% 
  separate(col = firm_feas_hs6, into = c("company_id", "hs6"), sep = "-") %>% 
  # Join import dummy variable by company, month, hs6
  left_join(imports_data_IDN %>% 
...
write_parquet(imports_grid_IDN, "../../Data/Indonesia/processed_data/imports_grid_product_model_IDN.parquet")
rm(imports_grid_IDN, grid_firm_hs6_imp_IDN, imports_data_IDN) # Remove objects to free space 
gc()
...
## Create all possible combinations between firms and FEASIBLE HS6 based on the HS4 code ----
grid_firm_hs6_exp_mitig_IDN<- exports_data_mitig_IDN %>% 
  select(company_id, hs4) %>% 
  # When a firm exports an HS6 product that belongs to a specific HS4 code, an observation is created for the corresponding firm-HS4 pair.
  distinct() %>% 
  # Expand the dataset to include all possible combinations between the firm and every HS6 code within the HS4 code that the firm exported.
...
exports_grid_mitig_IDN<- 
  CJ(firm_feas_hs6 = grid_firm_hs6_exp_mitig_IDN$firm_hs6, 
     date = unique(exports_data_mitig_IDN$date)) %>% 
  separate(col = firm_feas_hs6, into = c("company_id", "hs6"), sep = "-") %>% 
  # Join export dummy variable by company, month, hs6
  left_join(exports_data_mitig_IDN %>% 
              select(company_id, date, hs6, export_dummy),
            by = c('company_id', "date", "hs6")) %>% 
  # If missing, then product not imported by the company in that month
  mutate(export_dummy = ifelse(is.na(export_dummy), FALSE, export_dummy)) %>%
...
# Save imports grid in parquet file
write_parquet(exports_grid_mitig_IDN, "../../Data/Indonesia/processed_data/exports_grid_mitig_model_IDN.parquet")
rm(exports_grid_mitig_IDN, grid_firm_hs6_exp_mitig_IDN, exports_data_mitig_IDN) # Remove objects to free space 
gc()
...
## Create all possible combinations between firms and FEASIBLE HS6 based on the HS4 code ----
grid_firm_hs6_imp_mitig_IDN<- imports_data_mitig_IDN %>% 
  select(company_id, hs4) %>% 
  # When a firm exports an HS6 product that belongs to a specific HS4 code, an observation is created for the corresponding firm-HS4 pair.
  distinct() %>% 
  # Expand the dataset to include all possible combinations between the firm and every HS6 code within the HS4 code that the firm exported.
...
imports_grid_mitig_IDN<- 
  CJ(firm_feas_hs6 = grid_firm_hs6_imp_mitig_IDN$firm_hs6, 
     date = unique(imports_data_mitig_IDN$date)) %>% 
  separate(col = firm_feas_hs6, into = c("company_id", "hs6"), sep = "-") %>% 
  # Join import dummy variable by company, month, hs6
  left_join(imports_data_mitig_IDN %>% 
              select(company_id, date, hs6, import_dummy),
            by = c('company_id', "date", "hs6")) %>% 
  # If missing, then product not imported by the company in that month
  mutate(import_dummy = ifelse(is.na(import_dummy), FALSE, import_dummy)) %>%
