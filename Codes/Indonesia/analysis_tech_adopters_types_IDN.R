...
import_tech_mitig_IDN<- fread("../../Data/Indonesia/processed_data/imports_tech_mitigation_reg_firm_month_IDN.csv") 
...
export_tech_mitig_IDN<- fread( "../../Data/Indonesia/processed_data/exports_tech_mitigation_reg_firm_month_IDN.csv")



exp_adopters_type_summary<-
  export_tech_mitig_IDN %>% 
  select(company_id, adopter_type, covid_adopter_type, old_adopter_type) %>% 
  distinct(company_id, .keep_all = T) %>%
  select(company_id, covid_adopter_type, old_adopter_type) %>% 
  pivot_longer(
...
title <- textGrob("Technology Adopter Types Among Indonesian Importers and Exporters", gp=gpar(fontface="bold", fontsize=14), hjust = 0.5)

final_plot_IDN<-grid.arrange(p_exp, p_imp, ncol=2, top = title)
...
t_tests_old_covid_adopters_IDN<-
  conduct_t_tests_and_create_table(import_data = import_tech_mitig_IDN,
                                   export_data = export_tech_mitig_IDN,
...
t_tests_covid_early_late_adopters_IDN<-
  conduct_t_tests_and_create_table(import_data = import_tech_mitig_IDN,
                                   export_data = export_tech_mitig_IDN,
...
## Comparison between pre-2018 vs 2018-2019 adopters ----

# Since in Indonesia Bultwith dataset we only have data available from 2017 onwards, we do the comparison
# between pre-2018 adopters vs 2018 and 2019 adotpers
import_tech_mitig_IDN<-import_tech_mitig_IDN %>% 
  mutate(
    old_adopter_type = case_when(
      old_adopter_type == "2017_adopter" ~ "pre_2018_adopter", 
      old_adopter_type == "2018_adopter" ~ "2018_to_2019_adopter", 
      old_adopter_type == "2019_adopter" ~ "2018_to_2019_adopter", 
      TRUE ~ old_adopter_type
    )
  )


export_tech_mitig_IDN<-export_tech_mitig_IDN %>% 
  mutate(
    old_adopter_type = 
      case_when(
        old_adopter_type == "2017_adopter" ~ "pre_2018_adopter", 
...
t_tests_old_adopters_IDN<-
  conduct_t_tests_and_create_table(import_data = import_tech_mitig_IDN,
                                   export_data = export_tech_mitig_IDN,
                                   var_adopter_types = "old_adopter_type",
                                   adopter_types = c("pre_2018_adopter", "2018_to_2019_adopter"), 
                                   country = "Indonesia")
