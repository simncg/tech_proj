...
*
* Description: This do-file runs the IV analysis for Indonesia.
*                 
* Inputs: 
* 
* Outputs: 
...
foreach data of local imp_exp {
	...
	* Use Indonesia data at the firm-month level 
	import delimited "$path_data/Indonesia/processed_data/`data'_reg_firm_month_IDN.csv", clear
	...
	* Run regressions for model that measures if tech adoption affects trade outcomes 
	foreach iv of varlist  ltg dspg uspg ltb dspb uspb  {
		foreach covid_var of varlist cas sti dwc {
			...
			eststo `y'`iv'`covid_var': ivreghdfe `y' (tech = `iv'_`covid_var'), absorb(date company_id) cluster(company_id) savefirst savefprefix(f`y'`iv'`covid_var') 	
...
foreach y of local dep_vars {
	...
	* Creating tex file with the table for first stage
	cap file close fh 
	file open fh using "$path_outputs/Indonesia/regressions_results/firm_month_model/first_stage_log_iv_`y'_firm_month.tex", write replace  // Opening latex file for first stage
	...
	* Iterate over covid variables 
	foreach covid_var of varlist cas sti dwc {
		...
		* Create panel name  
		if("`covid_var'"== "cas"){
			file write fh "\hline \multicolumn{6}{l}{\textbf{COVID variable: Log(COVID Cases)}}\\" _n  
		}
...
foreach y of local dep_vars {
	...
	* Creating tex file with the table for first stage
	cap file close fh 
	file open fh using "$path_outputs/Indonesia/regressions_results/firm_month_model/second_stage_log_iv_`y'_firm_month.tex", write replace  // Opening latex file for first stage
